...
def get_candidate_set(var_info, var_round_n, var_question_gen_N=5):
    ...
    for name_var in vars:
        ...
        if var.startswith("Random_linespace_"):
            ...
            range_params = (
                var.split("_")[-1].replace("[", "").replace("]", "").split(",")
            )
            linespace = list(
                np.arange(
                    int(range_params[0]), int(range_params[1]), float(range_params[2])
                )
            )
            ...
            assert set_params.startswith("{") and set_params.endswith("}"), (
                "Invalid format for Random_Set"
            )
        ...
        if var_round_n == 0:
            ...
            candidate_set[name] = [
                np.round(float(x), var_round_n) for x in candidate_set[name]
            ]
...
def get_answer(candidate_set, var_answer, round_n):
    ...
    if var_answer.startswith("Fixed_Set_"):
        ...
        for ii in range(real_var_question_gen_N):
            ...
            try:
                answers.append(
                    float(np.round(eval(express), round_n))
                )  # TODO: consider sympify
            except Exception as e:
                print(f"Error evaluating expression '{express}': {e}")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Convert VAR-Math CSVs to JSONL. --datasets accepts space-separated or comma-separated list."
    )
    parser.add_argument(
        "--datasets",
        nargs="+",
        default=["amc23", "aime24", "aime25"],
        help="Datasets to process, e.g. --datasets amc23 aime24",
    )
    parser.add_argument(
        "--gen_n",
        type=int,
        default=5,
        help="Number of variable instances to generate.",
    )
    args = parser.parse_args()

    dataset_name_list = args.datasets
    # override global generation count
    var_question_gen_N = args.gen_n

    ### original data
    # for dataset_name in dataset_name_list:
    #     csv_file = os.path.join(data_dir, f"VAR-Math - {dataset_name}.csv")
    #     save_path = os.path.join(data_dir, f"{dataset_name}/test.jsonl")
    ...
    for dataset_name in dataset_name_list:
        csv_file = os.path.join(data_dir, f"VAR-Math - {dataset_name}.csv")
        save_path = os.path.join(data_dir, f"var_{dataset_name}/test.jsonl")
        ...
        for i in range(len(data)):
            ...
            if done == 1:
                ...
                candidate_set = get_candidate_set(var_info, var_round_n, var_question_gen_N)
